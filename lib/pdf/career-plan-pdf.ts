...
/**
 * The full AI Career Plan as a downloadable PDF (a JADE Pro feature).
...
function header(doc: Doc, data: PlanPdfData) {
  // Brand row: the JADE hexagon mark and wordmark.
  const top = MX - 8;
  doc.save().translate(MX, top).scale(0.75);
  doc.polygon([16, 2], [29, 10], [29, 22], [16, 30], [3, 22], [3, 10]);
  const grad = doc.linearGradient(3, 2, 29, 30);
  grad.stop(0, "#10b981").stop(0.55, "#22d3ee").stop(1, "#059669");
  doc.fill(grad);
  doc.restore();
  doc.font(BOLD).fontSize(15).fillColor(TEXT).text("JADE", MX + 30, top + 3, { lineBreak: false });
...
export function buildCareerPlanPdf(data: PlanPdfData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      bufferPages: true,
      info: { Title: `JADE AI Career Plan - ${toWinAnsi(data.careerTitle)}`, Author: "JADE" },
    });
    const chunks: Buffer[] = [];
    ...
    for (let i = 0; i < range.count; i++) {
      ...
      const by = data.model ? `Generated by ${data.model} on ${data.generatedOn}` : `Generated by JADE Planner on ${data.generatedOn}`;
      doc.text(`JADE · ${toWinAnsi(by)}`, MX, y, { lineBreak: false });
